...
async def callback_admin_promo_info(callback: CallbackQuery, db: Database):
    ...
    promocodes = await db.get_all_promocodes()
    promo = next((p for p in promocodes if p.id == promo_id), None)
    if not promo:
        await callback.answer("Промокод не найден", show_alert=True)
        return

    from ..keyboards.inline import admin_promo_list_keyboard
    status = "✅ Исчерпан" if promo.uses_count >= promo.max_uses else f"🟢 Активен ({promo.uses_count}/{promo.max_uses} использований)"
    text = (
